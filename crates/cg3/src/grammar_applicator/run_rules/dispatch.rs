...
impl crate::grammar_applicator::Engine<'_> {
    ...
    pub(crate) fn cohort_cb_dispatch(
        ...
        if rtype == KSelect || (rtype == KIff && !st.selected.is_empty()) {
            ...
            if st.selected.len() < treadings && !st.selected.is_empty() {
                ...
                let mut si = 0usize;
                let readings = self.doc.store.cohorts.get(target.0).readings.clone();
                for rd_orig in readings.iter().copied() {
                    ...
                    if let Some(sr) = sr_opt {
                        self.doc.store.readings.get_mut(sr.0).hit_by.push(rnumber);
                    }
                    if si < st.selected.len() && rd_orig == st.selected[si] {
                        si += 1;
                    } else {
                        self.doc.store.readings.get_mut(rd_orig.0).deleted = true;
                        drop.push(rd_orig);
                    }
                }
                // target->readings.swap(selected)
                {
                    ...
                    self.doc.store.cohorts.get_mut(target.0).readings = sel;
                }
            ...
            if cond {
                ...
                let mut oz = self.doc.store.cohorts.get(target.0).readings.len();
                while let Some(back) = st.removed.last().copied() {
                    self.doc.store.readings.get_mut(back.0).deleted = true;
                    let mut k = 0usize;
                    while k < oz {
                        if self.doc.store.cohorts.get(target.0).readings[k] == back {
                            oz -= 1;
                            self.doc
                                .store
                                .cohorts
            ...
            if let (Some(name), Some(value)) = (name, value) {
                ...
                *self.doc.variables.index_or_insert(nh.get()) = vh.get();
                if rflags.intersects(RF_OUTPUT) {
                    self.doc
                        .store
                        .single_windows
    ...
    fn rr_with(
        ...
        let current = st.current;
        let cur_was = self.scratch.current_rule;
        ...
        for sr in sub_rules {
            self.scratch.current_rule = Some(sr);
            st.rule = sr;
        ...
        self.scratch.current_rule = cur_was;
        st.rule = rule_was;
    ...
    fn rr_switchparent(&mut self, rule: RuleId) -> Result<(), crate::error::RunError> {
        ...
        self.doc.store.cohorts.get_mut(child.0).dep_parent = None;
        self.doc.store.cohorts.get_mut(parent.0).dep_parent = None;
        for &s in &siblings {
            self.doc.store.cohorts.get_mut(s.0).dep_parent = None;
        }
        if let Some(&gp) = grandparent_number.and_then(|g| self.doc.cohorts.cohort_map.get(&g)) {
    ...
    fn rr_restore(
        ...
        let list = match which {
            0 => self.doc.store.cohorts.get(cohort.0).delayed.clone(),
            1 => self.doc.store.cohorts.get(cohort.0).ignored.clone(),
            _ => self.doc.store.cohorts.get(cohort.0).deleted.clone(),
        };
        ...
        for rid in list {
            if self.does_set_match_reading(rid, maplist_num, false, false)? {
                ...
                self.doc.store.cohorts.get_mut(cohort.0).readings.push(rid);
                did_restore = true;
            } else {
                keep.push(rid);
            }
        ...
        match which {
            0 => self.doc.store.cohorts.get_mut(cohort.0).delayed = keep,
            1 => self.doc.store.cohorts.get_mut(cohort.0).ignored = keep,
            _ => self.doc.store.cohorts.get_mut(cohort.0).deleted = keep,
        }
        if did_restore {
            self.trace(rnumber, rsub_reading);
        }
    ...
    fn rr_fill_tag_list(&mut self, taglist: &mut TagList) -> Result<(), crate::error::RunError> {
        ...
        for &tt in taglist.iter() {
            let thash = self.grammar.single_tags_list.get(tt.0).hash;
            let ttype = self.grammar.tag_type(tt);
            let present = {
            ...
            if present {
                ...
                let stag = self.does_tag_match_reading(reading, tt, &tagv, false, true)?;
                if stag != 0 {
    ...
    fn rr_append(
        ...
        if self.doc.store.cohorts.get(cohort.0).readings.len() > 1 {
            let rs = self.doc.store.cohorts.get(cohort.0).readings.clone();
            let mut keep: ReadingList = Vec::new();
            for r in rs {
                if self.doc.store.readings.get(r.0).noprint {
                    let opt = Some(r);
    ...
    fn rr_copy(
        ...
        if childset1 != 0 {
            ...
            if rflags.intersects(RF_AFTER) {
                let mut cnt = 0;
                while at < self.doc.store.readings.get(creading.0).tags_list.len()
                    && cnt != spot_tags.len()
                {
                    at += 1;
                    cnt += 1;
    ...
    fn rr_substitute(
        ...
        let mut plain = true;
        let mut i = 0usize;
        while i < self.doc.store.readings.get(sr.0).tags_list.len() {
            let remter = self.doc.store.readings.get(sr.0).tags_list[i];
            if plain && !the_hashes.is_empty() && remter == the_hashes[0] {
                if self.doc.store.readings.get(sr.0).baseform == Some(TagHash(remter)) {
                    self.doc.store.readings.get_mut(sr.0).baseform = None;
                }
                self.doc.store.readings.get_mut(sr.0).tags_list[i] = substtag;
                tpos = i;
                let mut j = 1usize;
                while j < the_hashes.len() && i < self.doc.store.readings.get(sr.0).tags_list.len()
                {
                    let cur = self.doc.store.readings.get(sr.0).tags_list[i];
                    let tter = the_hashes[j];
                    if cur != tter {
                        plain = false;
                        break;
                    }
                    self.doc.store.readings.get_mut(sr.0).tags_list.remove(i);
                    self.doc.store.readings.get_mut(sr.0).tags.erase(tter);
                    if self.doc.store.readings.get(sr.0).baseform == Some(TagHash(tter)) {
                        self.doc.store.readings.get_mut(sr.0).baseform = None;
                    }
                    j += 1;
                }
            ...
            for &th in &the_hashes {
                ...
                tpos = i;
                self.doc.store.readings.get_mut(sr.0).tags_list[i] = substtag;
                self.doc.store.readings.get_mut(sr.0).tags.erase(th);
                if self.doc.store.readings.get(sr.0).baseform == Some(TagHash(th)) {
                    self.doc.store.readings.get_mut(sr.0).baseform = None;
                }
            }
            i += 1;
        ...
        if tpos != usize::MAX {
            if !plain {
                let mut k = 0usize;
                while k < self.doc.store.readings.get(sr.0).tags_list.len() && k < tpos {
                    if self.doc.store.readings.get(sr.0).tags_list[k] == substtag {
                        self.doc.store.readings.get_mut(sr.0).tags_list.remove(k);
                        tpos -= 1;
                    } else {
                        k += 1;
                    }
            ...
            let mapping_prefix = self.grammar.mapping_prefix;
            let mut idx = 0usize;
            while idx < self.doc.store.readings.get(sr.0).tags_list.len() {
                if self.doc.store.readings.get(sr.0).tags_list[idx] == substtag {
                    self.doc.store.readings.get_mut(sr.0).tags_list.remove(idx);
                    tpos = idx;
                    for t0 in &map_tags {
                        let mut tag = *t0;
                        ...
                        let thash = self.grammar.single_tags_list.get(tag.0).hash;
                        let ttype = self.grammar.tag_type(tag);
                        let first = self.grammar.single_tags_list.get(tag.0).tag.chars().next();
                ...
                && Some(wf) != parent_wf
            {
                ...
                for list_kind in 0..3 {
                    let rs = match list_kind {
                        0 => self.doc.store.cohorts.get(parent.0).readings.clone(),
                        1 => self.doc.store.cohorts.get(parent.0).deleted.clone(),
                        _ => self.doc.store.cohorts.get(parent.0).delayed.clone(),
                    };
                    for r in rs {
    ...
    ///
    /// DIVERGENCE (same behaviour, different mechanism): C++ clamps by WRITING
    /// `dep_target->offset` and restoring it after the scan. `dep_target` hangs
    ...
    fn rr_dep_relation(
        ...
        // The onward scan re-runs dep_target with its offset clamped to +/-1.
        let mut dep_ref = TestRef::new(dep_target);
        ...
        loop {
                ...
                && let Some(mut attach) = attach_out
            {
                ...
                let off = dep_ref.offset(&self.grammar.contexts_arena);
                if off != 0 {
                    dep_ref = dep_ref
                        .with_offset(&self.grammar.contexts_arena, if off < 0 { -1 } else { 1 });
                }
            } else {
                break;
            }
    ...
    fn rr_dep_target_cb(
        ...
        if rtype == KSetparent || rtype == KSetchild {
            self.doc.deps.has_dep = true;
            let attached = if rtype == KSetparent {
            ...
            if attached {
                let at_was = self.scratch.context_stack.last().unwrap().attach_to.cohort;
                self.scratch
                    .context_stack
                    .last_mut()
                    .unwrap()
        ...
        let is_plural = matches!(rtype, KAddrelations | KSetrelations | KRemrelations);
        ...
        for t0 in map_tags {
            ...
            let attach_gn = self.doc.store.cohorts.get(attach.0).global_number.get();
            match rtype {
                KAddrelation | KAddrelations => {
                    if !is_plural {
                        crate::cohort::set_related(&mut self.doc.store, attach);
                    }
                    crate::cohort::set_related(&mut self.doc.store, target);
