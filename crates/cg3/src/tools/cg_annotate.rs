...
/// to `fn` (binary), throwing on badbit/failbit — here, returning the failure.
fn file_save(path: &std::path::Path, data: &str) -> Result<(), AnnotateError> {
    std::fs::write(path, data.as_bytes()).map_err(|source| AnnotateError::Write {
...
/// Why no report could be written.
#[derive(Debug, thiserror::Error)]
enum AnnotateError {
    #[error(transparent)]
    Read(#[from] ProfileReadError),
    #[error("Output folder did not exist and could not be created! {path}: {source}")]
    FolderCreate {
        path: String,
        #[source]
        source: std::io::Error,
    },
    #[error("Error: cannot enter output folder {path}: {source}")]
    FolderEnter {
        path: String,
        #[source]
        source: std::io::Error,
    },
    #[error("Error: cannot write {path}: {source}")]
    Write {
        path: String,
        #[source]
        source: std::io::Error,
    ...
    #[error(
        "Error: the profile's grammar AST is malformed at byte {at}: no well-formed {}",
        .wanted.trim_start()
    )]
    Ast { wanted: &'static str, at: usize },
}

/// Per grammar id, the HTML to insert at each offset of its source.
type GrammarTags = BTreeMap<usize, BTreeMap<usize, VecDeque<String>>>;

// [spec:cg3:def:cg-annotate.main-fn+1]
// [spec:cg3:sem:cg-annotate.main-fn+1]
// [spec:cg3:req:robustness.cli-arguments]
/// C++ `int main(int argc, char* argv[])`.
///
/// DIVERGENCE: the C++ ignores `argc`, and ends by an uncaught exception on a
/// profile it cannot read, an output folder it cannot create or enter, a file
/// it cannot write and an AST it cannot parse. Each is reported here instead,
/// and the tool exits with `EXIT_FAILURE`.
pub fn main_annotate(args: &[String]) -> i32 {
    let [_, db, folder, ..] = args else {
        let program = args.first().map_or("cg-annotate", String::as_str);
        tracing::error!("USAGE: {} profile_db output_folder", basename(program));
        return EXIT_FAILURE;
    };
    match annotate(db, folder) {
        Ok(()) => EXIT_SUCCESS,
        Err(e) => {
            tracing::error!("{e}");
            EXIT_FAILURE
...
/// The run of [`main_annotate`]: read the profile at `db` and write its report
/// into `folder`.
fn annotate(db: &str, folder: &str) -> Result<(), AnnotateError> {
    // Profiler profiler; profiler.read(argv[1]);
    let profiler = read_profile(db)?;

    // fs::path folder(argv[2]); create + chdir.
    let path = std::path::Path::new(folder);
    if !path.exists() {
        std::fs::create_dir_all(path).map_err(|source| AnnotateError::FolderCreate {
            path: folder.to_string(),
            source,
        })?;
    }
    std::env::set_current_dir(path).map_err(|source| AnnotateError::FolderEnter {
        path: folder.to_string(),
        source,
    })?;
    ...
    let asts = split_grammar_asts(sget(0))?;
    let gs_tags = grammar_tags(&profiler, &asts)?;
    write_report(&profiler, &strings, &gs_tags)
}

/// Store per-grammar ASTs separately: split the profile's AST document into
/// one fragment per grammar, keyed by grammar id.
fn split_grammar_asts(mut ast: String) -> Result<BTreeMap<usize, String>, AnnotateError> {
    ...
    while let Some(sz) = ast.rfind("<Grammar ") {
        ...
        let e = ast[sz..].find("</Grammar>").ok_or(AnnotateError::Ast {
            wanted: "</Grammar>",
            at: sz,
        })? + sz;
        // auto g = ast.substr(sz, (e - sz) + 11); ast.erase(sz, (e - sz) + 11);
        // DIVERGENCE: the one byte after `</Grammar>` becomes one character,
        // so a multi-byte one is taken whole.
        let close = e + "</Grammar>".len();
        let end = close + ast[close..].chars().next().map_or(0, char::len_utf8);
        let g = ast[sz..end].to_string();
        ast.replace_range(sz..end, "");
        // sz = g.find(" u=\"") + 4; e = stoi(g.substr(sz, g.find("\"", sz) - sz));
        let gid: usize = attr(&g, 0, " u=\"")?;
        asts.insert(gid, g);
    }
    Ok(asts)
}

/// The number in the first attribute `name` (` b="`, with its quote) at or
/// after byte `from` of `ast` — the C++ `stoul(ast.substr(...))` over a
/// `find`, whose throws become [`AnnotateError::Ast`].
fn attr<T: std::str::FromStr>(
    ast: &str,
    from: usize,
    name: &'static str,
) -> Result<T, AnnotateError> {
    let malformed = || AnnotateError::Ast {
        wanted: name,
        at: from,
    };
    let start = ast[from..].find(name).ok_or_else(malformed)? + from + name.len();
    let len = ast[start..].find('"').ok_or_else(malformed)?;
    ast[start..start + len].parse().map_err(|_| malformed())
}

/// Extract each rule and context's start offset, per grammar.
fn grammar_tags(
    profiler: &Profiler,
    asts: &BTreeMap<usize, String>,
) -> Result<GrammarTags, AnnotateError> {
    // std::map<size_t, std::map<size_t, std::deque<std::string>>> gs_tags;
    let mut gs_tags: GrammarTags = BTreeMap::new();
    ...
    for (&git, ast) in asts {
        ...
        while let Some(found) = ast[last..].find(" l=\"") {
            ...
            let tagoff = ast[..last].rfind('<').ok_or(AnnotateError::Ast {
                wanted: "<",
                at: last,
            })? + 1;
            ...
            if tag == "Rule" || tag == "Context" {
                ...
                if let Some(entry) = profiler.entries.get(&k).copied() {
                    gs_tags
                        ...
                        .entry(b)
                        .or_default()
                        .push_back(entry_anchor(profiler, k.id, &entry, rid));
...
/// The anchor opening a rule's or context's annotation: a link to its usage
/// example page, classed good or bad by whether it matched, with its counts.
/// A context under a known rule (`rid`) shows its matches for that rule.
fn entry_anchor(profiler: &Profiler, id: u32, entry: &crate::profiler::Entry, rid: u32) -> String {
    let mut html = String::new();
    ...
    if entry.r#type == ET_RULE || rid == 0 {
        ...
        match profiler.rule_contexts.get(&(rid, id)).copied() {
            Some(second) if second != 0 => {
                html.push_str(r#" class="entry context good"><span class="stats">M:"#);
                html.push_str(&second.to_string());
            }
            _ => {
                html.push_str(r#" class="entry context bad"><span class="stats">M:0"#);
            }
        }
    }
    html.push_str("</span>");
...
/// Write the report's pages: one per grammar, one per usage example, the
