...
use crate::inlines::{read_le, write_le, write_utf8_le, ui16, ui32, ui8};
...
impl<'a> BinaryApplicator<'a> {
    ...
    pub fn read_window<R: Read>(&mut self, input: &mut R) -> Option<SwId> {
        ...
        let c_swindow = self.base.gWindow.alloc_append_single_window(&mut self.base.store);
        ...
        // 4. Window text / text_post.
        {
            ...
            let tp = read_str!();
            self.base.store.single_windows.get_mut(c_swindow.0).text_post = tp;
        }
        ...
        for cn in 0..cohort_count {
            ...
            if stag_count != 0 {
                ...
                for tn in 0..stag_count {
                    ...
                    self.base.add_tag_to_reading_rehash(wread, window_tags[ti], rehash);
                }
            }
            ...
            for _ in 0..reading_count {
                ...
                self.base.add_tag_to_reading(c_reading, window_tags[base_idx]);
                ...
                if !mappings.is_empty() {
                    self.base.split_mappings(&mut mappings, c_cohort, c_reading, true);
                }
                ...
                if prev.is_some() && (rflags & (BFR_SUBREADING as u16) != 0) {
                    ...
                    self.base.store.cohorts.get_mut(c_cohort.0).deleted.push(c_reading);
                } else {
...
impl BinaryFormat {
    ...
    pub fn bin_print_single_window<W: Write>(
        ...
        let all_cohorts: Vec<CohortId> = store.single_windows.get(window.0).all_cohorts.clone();
        for i in 0..all_cohorts.len() {
            ...
            if (ln == 0 || (ty & CT_REMOVED != 0)) && has_text {
                for j in (1..=i).rev() {
                    ...
                    store.cohorts.get_mut(prior.0).text.push_str(&txt);
                    store.cohorts.get_mut(cohort.0).text.clear();
                }
                let txt = store.cohorts.get(cohort.0).text.clone();
                store.single_windows.get_mut(window.0).text.push_str(&txt);
                store.cohorts.get_mut(cohort.0).text.clear();
            }
        }
        ...
        for cohort in all_cohorts {
            ...
            if dep_parent == 0 || dep_parent == DEP_NO_PARENT {
                ...
                if pr_local == 0 {
                    ...
                    wu32(&mut cohort_buffer, store.cohorts.get(pr.0).global_number);
                }
            } else {
            ...
            let relations: Vec<(u32, Vec<u32>)> = store
                .cohorts
                .get(cohort.0)
                .relations
                .iter()
            ...
            let mut readings: Vec<crate::arena::ReadingId> =
                store.cohorts.get(cohort.0).readings.clone();
            ...
            for top_reading in readings {
                if store.readings.get(top_reading.0).noprint {
                    continue;
                }
                ...
                while let Some(rid) = reading {
                    ...
                    let tags: Vec<u32> = store.readings.get(rid.0).tags_list.clone();
                    let parent_wf_hash = {
                    ...
                    reading = store.readings.get(rid.0).next;
                }
            }
