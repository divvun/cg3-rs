...
pub fn options_conv() -> options_conv_t {
    [
        ...
        uo("prefix", 'p', UOPT_REQUIRES_ARG, "sets the mapping prefix; defaults to @"),
        uo("in-auto", 'u', UOPT_NO_ARG, "auto-detect input format (default)"),
        uo("in-cg", 'c', UOPT_NO_ARG, "sets input format to CG"),
        uo3("v", 'v', UOPT_NO_ARG),
        uo("in-niceline", 'n', UOPT_NO_ARG, "sets input format to Niceline CG"),
        uo("in-apertium", 'a', UOPT_NO_ARG, "sets input format to Apertium"),
        uo("in-fst", 'f', UOPT_NO_ARG, "sets input format to HFST/XFST"),
        uo("in-plain", 'x', UOPT_NO_ARG, "sets input format to plain text"),
        uo("in-jsonl", 'j', UOPT_NO_ARG, "sets input format to JSONL (experimental, specs below)"),
        uo("in-binary", 'z', UOPT_NO_ARG, "sets input format to binary (experimental)"),
        uo("add-tags", '\0', UOPT_NO_ARG, "adds minimal analysis to readings (implies -x)"),
        uo("out-cg", 'C', UOPT_NO_ARG, "sets output format to CG (default)"),
        uo3("V", 'V', UOPT_NO_ARG),
        uo("out-apertium", 'A', UOPT_NO_ARG, "sets output format to Apertium"),
        uo("out-fst", 'F', UOPT_NO_ARG, "sets output format to HFST/XFST"),
        uo("out-matxin", 'M', UOPT_NO_ARG, "sets output format to Matxin"),
        uo("out-niceline", 'N', UOPT_NO_ARG, "sets output format to Niceline CG"),
        uo("out-plain", 'X', UOPT_NO_ARG, "sets output format to plain text"),
        uo("out-jsonl", 'J', UOPT_NO_ARG, "sets output format to JSONL (experimental, specs below)"),
        uo("out-binary", 'Z', UOPT_NO_ARG, "sets output format to binary (experimental)"),
        uo("wfactor", 'W', UOPT_REQUIRES_ARG, "FST weight factor (defaults to 1.0)"),
        uo("wtag", '\0', UOPT_REQUIRES_ARG, "FST weight tag prefix (defaults to W)"),
        uo("sub-delim", 'S', UOPT_REQUIRES_ARG, "FST sub-reading delimiters (defaults to #)"),
        uo("rtl", 'r', UOPT_NO_ARG, "sets sub-reading direction to RTL (default)"),
        uo("ltr", 'l', UOPT_NO_ARG, "sets sub-reading direction to LTR"),
        uo("ordered", 'o', UOPT_NO_ARG, "tag order matters mode"),
        uo("parse-dep", 'D', UOPT_NO_ARG, "parse dependency (defaults to treating as normal tags)"),
        uo("dep-delimit", '\0', UOPT_OPTIONAL_ARG, "delimit windows based on dependency; defaults to 10"),
        uo("unicode-tags", '\0', UOPT_NO_ARG, "outputs Unicode code points for things like ->"),
        uo("deleted", '\0', UOPT_NO_ARG, "read deleted readings as such, instead of as text"),
        uo("no-break", 'B', UOPT_NO_ARG, "inhibits any extra whitespace in output"),
    ]
}
