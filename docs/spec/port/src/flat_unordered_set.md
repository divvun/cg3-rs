...
> the next insert compacts it, so a stream that keeps removing and setting
> variables reaches that state and then hangs on setting one the removed set
> does not hold. When the bound is reached the port reclaims the tombstones
> with reserve(capacity()), the compaction insert() step (1) performs, and
> returns with nothing erased.
>
